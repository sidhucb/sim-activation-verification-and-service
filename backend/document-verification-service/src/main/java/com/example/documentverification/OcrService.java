...
public class OcrService {
    ...
    public DocumentDetails processDocument(String cardType, MultipartFile image1, MultipartFile image2)
        ...
        details.setPreparedAt(LocalDateTime.now()); // NEW: Set timestamp when document is processed
        
        if (extractedData != null) {
            details.setName(extractedData.getName()); // Use setName
            details.setCardNumber(extractedData.getCardNumber()); // Use setCardNumber
            details.setGender(extractedData.getGender()); // Use setGender
            details.setAddress(extractedData.getAddress()); // Use setAddress
            
            if (extractedData.getDob() != null) {
                try {
                    LocalDate dob = LocalDate.parse(extractedData.getDob(), DateTimeFormatter.ofPattern("dd/MM/yyyy"));
                    details.setDob(dob); // Use setDob
                    int age = Period.between(dob, LocalDate.now()).getYears();
                    details.setAge(age); // Use setAge
                } catch (Exception e) {
                    System.err.println("Error parsing DOB from LLM output: " + extractedData.getDob() + " - " + e.getMessage());
                    details.setDob(null); // Ensure DOB is null if parsing fails
                    details.setAge(null); // Ensure Age is null if DOB parsing fails
                }
            }
        }
    ...
    private BufferedImage toBufferedImage(MultipartFile file) throws IOException {
        ...
        if (image == null) {
            ...
            try (ByteArrayOutputStream os = new ByteArrayOutputStream()) {
                ...
                image = ImageIO.read(file.getInputStream()); // Fallback
            }
        }
        if (image == null) {
